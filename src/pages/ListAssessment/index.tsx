...
const useQuery = (): AssessmentQueryCondition => {
  ...
  return useMemo(() => {
    return { ...clearNull({ query_key, page, order_by, query_type, teacher_name, assessment_type, status, class_id, due_at_le, only_current_user }) };
 ...
 useEffect(() => {
  const { only_current_user } = condition;
    if(only_current_user === "true") {
      (async () => {
        const { payload } = (await dispatch(getCurrentUserInfo())) as unknown as PayloadAction<AsyncTrunkReturned<typeof getCurrentUserInfo>>;
