...
function trigger(name) {
  ...
  if (regs.length) {
    results = regs.map(function (r) {
      ...
      return !(0, _utils.isUndefined)(result);
    }).map(function (result) {
      return (0, _isPromise["default"])(result) ? result : Promise.resolve(result);
    });
  }
...
var defineLifeData = function defineLifeData(target, options) {
  ...
  Object.defineProperties(target, _objectSpread(_defineProperty({}, _consts.LESYM, {
    value: Object.seal({
      registry: registry,
      events: events,
      options: options,
...
var addLife = function addLife(target) {
  ...
  target = target || {};
  options = _objectSpread(_objectSpread({}, _defaults["default"]), options);
